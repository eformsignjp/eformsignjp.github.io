...
eformsign で使用する **電子文書の書式** です。契約書、申込書、同意書などのように従来は紙で書かれていたものを電子書式の形に変換したものをテンプレートと言います。テンプレートは、eformsign サービスの **Web フォームデザイナー** 、または Microsoft Office に Add-in される **フォームビルダー** を使用して作成およびアップロードすることができます。フォームビルダーを使用したテンプレートの設定および管理する方法については、 `フォームビルダー <#template_fb>`__\ をご参照ください。
