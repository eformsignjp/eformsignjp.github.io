...
eformsign サービスの **Web フォームデザイナー** 、または Microsoft
Office に Add-in される **フォームビルダー**
を使用して作成およびアップロードすることができます。フォームビルダーを使用したテンプレートの設定および管理する方法については、
...
1. 作成したい PDF
   ファイルをテンプレート作成カードに直接ドラッグするか、\ **PCからファイルを選択**
...
2. 目的のテンプレートの **名前** または **設定**
   アイコン（\ |image1|\ ）をクリックします。
...
**{{**
   ...
   **文書タイトルの自動生成ルール** の入力欄に **{{**
   を入力すると、テンプレートに追加されているコンポーネントの **名前**
   のリストが表示され、目的の **名前**
...
-  **文書リストで文書番号を確認する**
   ...
   文書番号は、文書リストを表示する
   **文書トレイ**\ （要処理文書、進行中文書、完了文書）と **文書管理**
   メニュー（文書の管理権限が必要）で確認できます。
...
-  **属性**
   では、ステップ名と状態の設定、および各ステップの設定を細部設定できます。
...
-  **文書番号の設定を変更：テンプレート設定**
   で設定した文書番号の設定を変更できる機能です。文書番号の採番が設定されているテンプレートの開始番号を再設定できます。
